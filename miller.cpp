...
void ff_lttp_Costello_lazy_montgomery_0(fp8_t *f, efp2_jacobian_t *U, efp_t *S){
  ...
  fp2_mul_base(&tmpD_fp2, &tmpC_fp2);
  // fp2_set(&tmpD_fp2, &tmpC_fp2);//here
...
void miller_opt_ate_jac(fp8_t *f,efp8_t *P,efp8_t *Q){
    ...
    efp8_to_Jacefp2(&S,Q);
    // printf("defQ::"),efp2_checkOnTwsitCurve(&mapped_Q),getchar();
    // printf("defS::"),efp2_jacobi_checkOnCurve_Twist(&S),getchar();

    mp_bitcnt_t i;
    for(i=(mpz_sizeinbase(miller_loop_s,2)-2);i!=-1;i--){
      ff_lttp(f,&S,&mapped_P);
      // efp2_jacobi_checkOnCurve_Twist(&S),getchar();
