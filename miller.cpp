...
void ff_lttp(fp8_t *f, efp2_jacobian_t *S, efp_t *P){
  ...
  fp_mul(&tmp1_fp8.x0.x1,&nextZ,&tmp1_fp);           // = nextZ*Z^2
  fp_mul_base_inv(&tmp1_fp8.x0.x1,&tmp1_fp8.x0.x1);  // = nextZ*Z^2*(2^-1)
  fp_mul(&tmp1_fp8.x0.x1,&tmp1_fp8.x0.x1,&P->y);     // = nextZ*Z^2*(2^-1)*Py

  fp_mul(&tmp3_fp,&t3,&S->x);                        //tmp3 = t3*X
  fp_sub(&tmp1_fp8.x0.x0,&tmp3_fp,&tmp2_fp);         // = t3*X - 2*t1

  fp_mul(&tmp1_fp8.x1.x1,&t3,&P->x);                 // = t3*Px
  fp_mul(&tmp1_fp8.x1.x1,&tmp1_fp8.x1.x1,&tmp1_fp);  // = t3*Px*Z^2
  fp_set_neg(&tmp1_fp8.x1.x1,&tmp1_fp8.x1.x1);       // = -t3*Px*Z^2
  fp_mul_base_inv(&tmp1_fp8.x1.x1,&tmp1_fp8.x1.x1);  // = -t3*Px*Z^2*(2^-1)

  fp8_mul_sparse_dbl(f,&tmp1_fp8,f);        //Capable for further Karatsuba //update
...
//add line 
void ff_ltqp(fp8_t *f, efp2_jacobian_t *S, efp_t *Q,efp_t *P){
  static fp_t tmp1_fp, tmp2_fp,tmp3_fp;
  ...
  fp_mul_base_inv(&tmp1_fp8.x2.x1,&nextZ);
  // fp_println("&P->y", &P->y);
  fp_mul(&tmp1_fp8.x2.x1,&tmp1_fp8.x2.x1,&P->y);
  // fp_println("&tmp1_fp8.x2.x1", &tmp1_fp8.x2.x1);

  fp_mul(&tmp1_fp8.x0.x0,&t2,&P->x);
  fp_set_neg(&tmp1_fp8.x0.x0,&tmp1_fp8.x0.x0);

  fp_mul(&tmp1_fp,&t2,&Q->x);
  fp_mul(&tmp2_fp,&nextZ,&Q->y);
  fp_sub(&tmp1_fp8.x2.x0,&tmp1_fp,&tmp2_fp);
  
  // fp8_println("tmp1_fp8", &tmp1_fp8);
  fp8_mul_sparse_add(f,&tmp1_fp8,f); //Capable for further Karatsuba //update
...
// void ff_lttp_lazy_montgomery2(fp8_t *f, efp_jacobian_t *S, efp_t *P){
//   fp8_sqr_lazy_montgomery(f,f); //update
//   static fp_t tmp1_fp, tmp2_fp,tmp3_fp;
...
//   //0.0
//   fp_sub_nonmod_single(&tmp1_fp8.x0.x0, &A2_fp, &B1_fp);
//   fp_l1shift_nonmod_single(&tmp1_fp8.x0.x0, &tmp1_fp8.x0.x0);
//   fp_add_nonmod_single(&tmp1_fp8.x0.x0, &tmp1_fp8.x0.x0, &A2_fp);

//   //0.1
//   fp_mulmod_montgomery(&tmp1_fp8.x0.x1,&tmp3_fp,&P->y);     // = nextZ*Z^2*(2^-1)*Py
//   fp_mul_base_inv_single(&tmp1_fp8.x0.x1,&tmp1_fp8.x0.x1);  // = nextZ*Z^2*(2^-1)
